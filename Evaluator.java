...
public class Evaluator {
  public static final int n = 43;
	...
	public static List<List<Integer>> getCliques(String bitString) {
		charBits = bitString.toCharArray();
		bits = charToBoolean(charBits);
		adj = getAdjMatrixFromBoolArray(bits);
		...
		for (int i = 0; i < upperBound; i++) {
			...
			if (result == 0 || result == 10) {
				cliques.add(new ArrayList<Integer>(Arrays.asList(arr)));
			}
		}
	 ...
	 */
	private static int evalEdges(Integer[] arr) {
		int result = 0;
	 ...
	 */
	private static void getElement(int m, Integer[] arr) {
		int a = n;
		int b = k;
